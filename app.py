...
def signup():
    if request.method == 'POST':
        try:
            ...
            login_user(new_user)
            app.logger.debug(f"User {username} signed up and logged in successfully.")
...
def choose_subject():
    if request.method == 'POST':
        try:
            ...
            app.logger.debug(f"User {current_user.username} selected subjects: {selected_subjects}")
            ...
            app.logger.error(f"Error occurred during subject selection: {e}")
            flash('An error occurred while choosing a subject. Please try again.', 'error')
...
def upindex():
    ...
    return render_template('upindex.html', posts=posts)
...
def upload():
    ...
    if file:
        ...
        file.save(file_path)
        new_post = Post(image_filename=filename, text=text)
        db.session.add(new_post)
        db.session.commit()
        return redirect(url_for('upindex'))
